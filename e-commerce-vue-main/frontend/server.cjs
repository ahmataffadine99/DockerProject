...
// Proxy Auth Service
app.use('/api/auth', createProxyMiddleware({
  target: 'http://auth-service:3001', // Utiliser le nom du service
  changeOrigin: true,
  logLevel: 'debug',
  pathRewrite: (path, req) => {
    // Restaurer le chemin complet
...
// Product Service
app.use('/api/products', createProxyMiddleware({
  target: 'http://product-service:3000', // Utiliser le nom du service
  changeOrigin: true,
  logLevel: 'debug',
  onProxyReq: (proxyReq, req) => {
    console.log(`[PROXY] Forwarding request to product-service: ${req.method} ${req.url}`);
...
// Order Service
app.use('/api/orders', createProxyMiddleware({
  target: 'http://order-service:3002', // Utiliser le nom du service
  changeOrigin: true,
  logLevel: 'debug',
  onProxyReq: (proxyReq, req) => {
    console.log(`[PROXY] Forwarding request to order-service: ${req.method} ${req.url}`);
...
// Cart Service
app.use('/api/cart', createProxyMiddleware({
  target: 'http://order-service:3002', // Utiliser le nom du service order service
  changeOrigin: true,
  logLevel: 'debug',
  onProxyReq: (proxyReq, req) => {
    console.log(`[PROXY] Forwarding request to cart-service: ${req.method} ${req.url}`);
